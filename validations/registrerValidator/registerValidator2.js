...
//Patron para la contraseña (debe tener minimamente minuscula, mayuscula, numeros y que tenga entre 7 y 30 caracteres, los caracteres especiales son opcionales)
const password_regex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*[\d])(?=.*[^\w\d\s]).{7,30}$/

//Objeto que valida cada input del formulario recibido
const Registro2 = z.object({
    //Nombre de usuario
    username: z.string().regex(username_regex, "Nombre de usuario -> Solo admite una letra como primer caracter, seguido de caracteres, numeros, puntos, guiones bajos y entre 4 y 20 caracteres"),
